...
if(isset($_POST["id_materia"])){
    $query = $conexion->prepare("SELECT Calificacion.*, tema, nombre_materia FROM Calificacion 
    ...
    INNER JOIN Usuario ON Usuario.id_usuario = Materia.id_usuario 
    WHERE Usuario.usuario = ? AND Materia.id_materia = ?");
    $query->bindParam(1, $_SESSION['usuario']);
...
foreach ($evaluaciones as $row) { 
    $html .= "<tr><td>".$row['nombre_materia']."</td><td>".$row['tema']."</td><td>".$row['usuario']."</td><td>".$row['calificacion']."</td>
